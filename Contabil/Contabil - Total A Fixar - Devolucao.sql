...
SELECT
    ...
    SUM(
    (DADOS_GERAIS.VALORUNITARIO) --/ (DADOS_GERAIS.CONTAGEM)
    * 
    DIVIDE(
        (DADOS_GERAIS.AFIXAR_QTD) - COALESCE(DADOS_GERAIS.FIXACAO_QTD,0)
        ,60)
    ) -
    SUM(DADOS_GERAIS.VALORUNITARIO) / SUM(DADOS_GERAIS.CONTAGEM)* DIVIDE(COALESCE(CONF_1041.QTDBX,0),60)
