...
SALDO AS (
    ...
    ROW_NUMBER() OVER (
        PARTITION BY ITEMSALDO.ESTAB, ITEMSALDO.LOCAL, ITEMSALDO.ITEM, ITEMSALDO.CODIGOSALDO
        ORDER BY ITEMSALDO.ANO DESC, ITEMSALDO.MES DESC
    ) AS RK
    FROM ITEMSALDO
    INNER JOIN ITEMAGRO ON ITEMAGRO.ITEM = ITEMSALDO.ITEM
    ...
    AND (0 IN (:GRUPOCTB) OR ITEMGRUPO.GRUPOCONTABIL IN (:GRUPOCTB))
...
BASE AS (
    ...
    INNER JOIN SALDO ON
        ...
        AND SALDO.ANO = ITEMSALDO.ANO
        AND SALDO.MES = ITEMSALDO.MES
        AND SALDO.RK = 1
...
TERCEIROS AS (
    ...
    NFCAB.NOTACONF,
    NFITEM.QUANTIDADE - COALESCE(BAIXAS.QTD,0) AS SALDO,
    ARREDONDAR(
        DIVIDE(
...
SALDO_EMTERC,
CUSTO_EMTERC,
SALDO_CTB - SALDO_EMTERC AS SALDOFIS,
CUSTO_CTB - CUSTO_EMTERC AS CUSTOFIS,
SALDO_DETERC,
CUSTO_DETERC
FROM(
SELECT
...
0 SALDO_CTB,
0 CUSTO_CTB,
CASE WHEN NOTACONF IN (42,43,223,225,232,246,1013) AND CODIGOSALDO IN (4) THEN SALDO ELSE 0 END SALDO_EMTERC,
CASE WHEN NOTACONF IN (42,43,223,225,232,246,1013) AND CODIGOSALDO IN (4) THEN CUSTO ELSE 0 END AS CUSTO_EMTERC,
0 SALDO_DETERC,
0 CUSTO_DETERC
FROM TERCEIROS

WHERE SALDO <> 0 AND CODIGOSALDO IN (4)
...
0 SALDO_EMTERC,
0 CUSTO_EMTERC,
SALDO AS SALDO_DETERC,
CUSTO AS CUSTO_DETERC
FROM TERCEIROS
WHERE SALDO <> 0 AND CODIGOSALDO IN (13)
