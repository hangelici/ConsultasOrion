...
M.QUANTIDADE,
IMP.VALORIMPOSTO,
NFITEM.VALORTOTAL - IMP.VALORIMPOSTO - NVL(IMP.VALORDESONERACAO,0) AS VLR_CTB,
ARREDONDAR(NFITEM.CUSTOCMV*M.QUANTIDADE,2) - (NFITEM.VALORTOTAL - IMP.VALORIMPOSTO - NVL(IMP.VALORDESONERACAO,0)) as DIF,
SUM(CASE
...
FROM BASE
WHERE
BASE.SALDO_QTD = 0 AND BASE.NATUREZADAOPERACAO = 'DC' AND BASE.DATAMOVIM >= :DTBASE AND BASE.DIF <> 0
)
SELECT
D.ESTAB,
D.ITEM,
