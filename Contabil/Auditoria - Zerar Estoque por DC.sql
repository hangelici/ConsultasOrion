...
D.DIF,
--
(D.VALORTOTAL - SUM(NFITEMIMPOSTO.VALORTRIBISENTAS) OVER(PARTITION BY NFITEMIMPOSTO.ESTAB, NFITEMIMPOSTO.SEQNOTA)) AS VALORTOTAL_LIQ,
(D.VLR_CTB - SUM(NFITEMIMPOSTO.VALORTRIBISENTAS) OVER(PARTITION BY NFITEMIMPOSTO.ESTAB, NFITEMIMPOSTO.SEQNOTA)) AS VLRCTB_LIQ,
((D.VLR_CTB - SUM(NFITEMIMPOSTO.VALORTRIBISENTAS) OVER(PARTITION BY NFITEMIMPOSTO.ESTAB, NFITEMIMPOSTO.SEQNOTA)) - D.CUSTO_NOTA) AS DIF_LIQ
FROM DADOS D

/*ALTERAÇÕES*/
LEFT JOIN NFITEMIMPOSTO
    ON  D.ESTAB = NFITEMIMPOSTO.ESTAB
    AND D.SEQ = NFITEMIMPOSTO.SEQNOTA
/*ALTERAÇÕES*/
