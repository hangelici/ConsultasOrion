...
cursor_oracle.execute("""
    select
    cnpjf,INSCESTAD,PRODUTOR,0 as SEQENDERECO
    from contamov
    where INSCESTAD is not NULL
    union all
    SELECT
    cnpjf,CREDENCIALAGRO AS INSCESTAD,PRODUTOREND AS PRODUTOR,SEQENDERECO
    from endereco                
""")
pessoa_dict = {
...
for row in resultado_final:
    ...
    # ---------------- CONTRATO ----------------
    if (emitid, emitie) in contrato_dict:
        estab_contrato, contrato, contconf = contrato_dict[(emitid, emitie)]
    else:
...
for row in resultado_final_otimizado:
    ...
    ncm_valid      = lista[-14]
    produtor       = lista[-13]
    seqendereco    = lista[-12]
    idcarga        = lista[-11]
    localestoque   = lista[-10]
    dif_peso       = lista[-9]
    dif_peso_app   = lista[-8]
    estab_contrato = lista[-7]
    contrato       = lista[-6]
    confcont       = lista[-5]
    notaref        = lista[-2]
    classestoque   = lista[-1]
