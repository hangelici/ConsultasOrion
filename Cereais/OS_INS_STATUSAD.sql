...
V_PIX VARCHAR2(20); 
V_USER VARCHAR2(80);
V_PEDIDO VARCHAR2(80);
V_VALIDAPEDIDO NUMBER;
BEGIN
            V_PEDIDO := 'Aprovação via SGI: Pedido:'|| :NEW.NUMERO ||' Serie:'|| :NEW.SERIE;

            BEGIN
                SELECT DISTINCT
                1 INTO V_VALIDAPEDIDO
                FROM CONTAMOVLAN
                WHERE 
                CONTAMOVLAN.HISTORICO = V_PEDIDO;
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    V_VALIDAPEDIDO := 0;
            END;

            IF :NEW.STATUS_AD = 'PENDENTE DOCUMENTO' AND NVL(:OLD.STATUS_AD,'N') <> 'PENDENTE DOCUMENTO' AND V_VALIDAPEDIDO = 0 THEN
