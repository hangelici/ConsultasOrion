...
BEGIN
   ...
   CONTRATOCONF := CASE WHEN :NEW.PEDIDOCONF = 200 THEN 1 WHEN :NEW.PEDIDOCONF = 204 THEN 7 WHEN :NEW.PEDIDOCONF = 202 THEN 50 ELSE 2 END;
    ...
    SELECT 
        CASE 
            WHEN (FORMAPGTO IN (24,22,20,21,26,27) AND (PRAZOPAGAMENTO > CURRENT_DATE)) THEN PRAZOPAGAMENTO
            ELSE CURRENT_DATE 
 ...
 --  SELECT CASE WHEN (FORMAPGTO IN (24,22,20) and (prazopagamento <= current_date + 2)) THEN current_date ELSE prazopagamento end into prazopagamento FROM pedcabpgto where pedcabpgto.estab=:NEW.ESTAB AND pedcabpgto.SERIE=:NEW.SERIE AND pedcabpgto.NUMERO=:NEW.NUMERO;
  ...
  BEGIN
   ...
   OS_PESS_TRIANGULAR
    ...
    v_COD_PRODUTOR,v_NOME_PRODUTOR,v_CNPJF_PRODUTOR,v_INSCESTAD_PRODUTOR,v_triangular,v_pess_triangular
    from pedcab_u
   where pedcab_u.estab=:NEW.ESTAB and pedcab_u.serie=:NEW.SERIE and pedcab_u.numero=:NEW.NUMERO; 

   IF P_INSECAOTAXA = 'S' THEN
   BEGIN
    SELECT COD_UMIDADE
        INTO V_UMID_CTR
    FROM U_TABUMIFIL_ITE
    WHERE U_TABUMIFIL_ITE.ESTAB = :NEW.ESTAB
    AND U_TABUMIFIL_ITE.CONFIG_CONTRATO = CONTRATOCONF
    AND U_TABUMIFIL_ITE.ITEM = ITEM;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            V_UMID_CTR := NULL;
   END;
   END IF;
    ...
    BEGIN
    IF v_prazo IS NOT NULL AND TRUNC(v_prazo) < TRUNC(SYSDATE) THEN
        v_prazo := TRUNC(SYSDATE);
    END IF;
    END;
   ...
   contrato_edit,NRTICKET,INSECAOTAXA,ESTABORIGEM, ATIVO_OS,REGIAO,COD_PRODUTOR,NOME_PRODUTOR,CNPJF_PRODUTOR,INSCESTAD_PRODUTOR,OS_TRIANGULAR,OS_PESS_TRIANGULAR,OS_TABUMIDADE,
   QTDORI)   
  ...
  -- 'AJUSTE','AJUSTE','AJUSTE','AJUSTE',
   ...
   v_COD_PRODUTOR,v_NOME_PRODUTOR,v_CNPJF_PRODUTOR,v_INSCESTAD_PRODUTOR,v_triangular,v_pess_triangular,V_UMID_CTR,
   QUANTIDADE
   );   
