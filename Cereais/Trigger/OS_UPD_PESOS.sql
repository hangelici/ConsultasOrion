...
BEGIN
    ...
    */
    ...
    IF V_FILIAL = V_CNPJ AND :NEW.DT_INCLUSAO IS NOT NULL AND :NEW.CODTERMINAL NOT IN (139917, 141456) THEN
        ...
        IF V_VALIDA_UPD = 'S' AND ( (NVL(:NEW.PLIQUIDO,0) <> NVL(V_DESCARGA,0)) OR (NVL(:NEW.RETENCAO,0) <> NVL(V_RETENCAO,0)) ) THEN
            V_LOG := 'S';
            UPDATE RETENPORTO
            SET PESODESCARREGAMENTO =
                    CASE
                        WHEN NVL(:NEW.PLIQUIDO,0) <> NVL(V_DESCARGA,0)
                        THEN :NEW.PLIQUIDO
                        ELSE PESODESCARREGAMENTO
                    END,
                PESORETENCAO =
                    CASE
                        WHEN NVL(:NEW.RETENCAO,0) <> NVL(V_RETENCAO,0)
                        THEN :NEW.RETENCAO
                        ELSE PESORETENCAO
                    END
    ...
    IF V_LOG = 'S' THEN
    /* ###### Validações Ticket 1273853 ###### */
        INSERT INTO U_LOG_TRADING (ESTAB, SEQNOTA, CODTERMINAL, CODFORNECEDOR, CHAVEACESSO, PESO_ANT, PESORETENCAO_ANT, PESODESCARREGAMENTO_ANT, QUEBRA_ANT, QUEBRA_NOVO, PESORETENCAO_NOVO, PESODESCARREGAMENTO_NOVO, MOTIVO, DT_INSERT)
        VALUES (:NEW.ESTAB, V_SEQNOTA, :NEW.CODTERMINAL, :NEW.CODFORNECEDOR, :NEW.CHAVEACESSO, V_PESO, V_RETENCAO, V_DESCARGA, V_QUEBRA, :NEW.QUEBRA_SOBRA, :NEW.RETENCAO, :NEW.PLIQUIDO, V_MOTIVO, CURRENT_TIMESTAMP);
    END IF;
