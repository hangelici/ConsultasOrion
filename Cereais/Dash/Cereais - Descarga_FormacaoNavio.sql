WITH BASE AS
    ...
    CHAVEACESSO,
    CODFORNECEDOR,
    NVL(SEQNOTA_388, 0) AS SEQNOTA_388
...
RETENCAO AS
    ...
    SELECT
    BASE.CHAVEACESSO,
    NVL(SUM(PESORETENCAO),0) AS PESORETENCAO
    FROM RETENPORTO R
    INNER JOIN NFCAB ON
    NFCAB.ESTAB = R.ESTAB
    AND NFCAB.SEQNOTA = R.SEQNOTA
    INNER JOIN BASE ON BASE.CHAVEACESSO = NFCAB.CHAVEACESSONFE
    GROUP BY BASE.CHAVEACESSO
    ),
...
FRETE AS
    ...
    FROM NFCAB
    INNER JOIN BASE ON BASE.CHAVEACESSO = NFCAB.CHAVEACESSONFE
    INNER JOIN CONTRATONFITE C ON
    C.ESTAB = NFCAB.ESTAB
    AND C.SEQNOTA = NFCAB.SEQNOTA
    INNER JOIN CONTRATO_U ON
    CONTRATO_U.ESTAB = C.ESTAB
    AND CONTRATO_U.CONTRATO = C.CONTRATO
    ),
...
COMPLEMT AS
    ...
    NF.CHAVEACESSONFE,
    NF.ESTAB,
    SUM(DISTINCT NFITEM.QUANTIDADE) AS NF_C
    FROM NFCAB
    INNER JOIN NFCAB_U ON
    NFCAB_U.ESTAB = NFCAB.ESTAB AND
    NFCAB_U.SEQNOTA = NFCAB.SEQNOTA AND 
    NFCAB_U.OS_NOTAORIGEM > 0 AND 
    NFCAB_U.OS_NOTAORIGEM <> 0
    INNER JOIN NFITEM ON 
    NFITEM.ESTAB = NFCAB.ESTAB
    AND NFITEM.SEQNOTA = NFCAB.SEQNOTA
    INNER JOIN NFCAB NF ON
    NF.NOTA = NFCAB_U.OS_NOTAORIGEM AND
    NF.ESTAB = NFCAB_U.ESTAB
...
DESCARGA AS
    ...
    WHERE
    NFCAB.STATUS <> 'C'
    GROUP BY NFCAB.ESTAB,NFCAB.CHAVEACESSONFE,ITEMAGRO.ITEM,ITEMAGRO.DESCRICAO,NFCFG.NOTACONF,NFCFG.DESCRICAO,NFCAB.SEQNOTA,
    BASE.SEQNOTA_388
    ),
...
REF1 AS
    ...
    U.DATA_CCT AS DTCCT,
    U.SEQ_END_TERMINA AS SEQ_END_TERM1,
    U.DT_INCLUSAO AS DT_INCLUSAO_1
    ...
    AND (U.CODTERMINAL NOT IN (80861,28910,40499/*53142*/))
    ),
...
REF2 AS
    ...
    X.RETENCAO AS RET2,
    X.SEQ_END_TERMINA AS SEQ_END_TERM2,
    X.DT_INCLUSAO AS DT_INCLUSAO_2
    ...
    WHERE
    X.CODTERMINAL IN (80861,28910,40499/*53142*/)
    AND X.CHAVEACESSO IS NOT NULL
    )
    
    
    
SELECT DISTINCT/*+ LEADING(D) USE_HASH(COMPLEMT) */
    D.CHAVEACESSONFE,
    U_NOTASSIEG_ITENS.CFOP AS CFOP_FISCALIO,
    U_NOTASSIEG_ITENS.UNID_TRIB AS UNIDADE_TRIBUTAVEL,
    U_NOTASSIEG_ITENS.QTD_TRIB AS QUANTIDADE_TRIBUTAVEL,
    U_NOTASSIEG_ITENS.NCM AS NCM_FISCALIO,
    --
    SIEG.CHAVEACESSONFE AS CHAVE_PRODUTOR,
    SIEG_ITENS.CFOP AS CFOPPROD_FISCALIO,
    SIEG_ITENS.UNID_TRIB AS UNIDADE_TRIBUTAVELPROD,
    SIEG_ITENS.QTD_TRIB AS QUANTIDADE_TRIBUTAVELPROD,
    SIEG_ITENS.NCM AS NCMPROD_FISCALIO,
    --
    D.QTD_NOTA, 
    REF1.ESTAB,
    D.ITEM,
    ...
    D.NF_QB, 
    D.NF_RET,
    NVL(D.NF_RET + D.NF_QB + D.NF_CD - D.QTD_NOTA + RETENCAO.PESORETENCAO,0) AS DIF_APARTIR,
    D.QTD_NOTA - NVL(REF2.PESO2,REF1.PESO1) AS DIF_QTD,
    NVL(D.NF_CD,0) 
    + NVL(REF1.QUEBRA1,0) 
    + NVL(REF2.QUEBRA2,0) 
    - NVL(COMPLEMT.NF_C,0) AS DIF_QDC,
...
FROM DESCARGA D
    ...
    LEFT JOIN FRETE ON FRETE.CHAVEACESSONFE = D.CHAVEACESSONFE
    LEFT JOIN RETENCAO ON RETENCAO.CHAVEACESSO = D.CHAVEACESSONFE
    LEFT JOIN COMPLEMT ON COMPLEMT.CHAVEACESSONFE = D.CHAVEACESSONFE
    LEFT JOIN CONTRATONFITE CNF ON CNF.ESTABNOTA = D.ESTAB AND CNF.SEQNOTA = D.SEQNOTA AND CNF.SEQNOTAITEM = 1
    --
    LEFT JOIN U_NOTASSIEG ON D.CHAVEACESSONFE = U_NOTASSIEG.CHAVEACESSONFE AND D.ESTAB = U_NOTASSIEG.ESTAB
    LEFT JOIN U_NOTASSIEG_ITENS ON U_NOTASSIEG.CHAVEACESSONFE = U_NOTASSIEG_ITENS.CHAVE
    --
    LEFT JOIN NFCABPRODUTOR ON D.ESTAB = NFCABPRODUTOR.ESTAB AND D.SEQNOTA = NFCABPRODUTOR.SEQNOTA
    LEFT JOIN U_NOTASSIEG SIEG ON NFCABPRODUTOR.CHAVEACESSONFP = SIEG.CHAVEACESSONFE AND NFCABPRODUTOR.ESTAB = SIEG.ESTAB
    LEFT JOIN U_NOTASSIEG_ITENS SIEG_ITENS ON SIEG.CHAVEACESSONFE = SIEG_ITENS.CHAVE AND SIEG.ESTAB = SIEG_ITENS.ESTAB
WHERE
    (
