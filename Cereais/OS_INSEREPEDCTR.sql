...
BEGIN
 ...
 --  SELECT CASE WHEN (FORMAPGTO IN (24,22,20) and (prazopagamento <= current_date + 2)) THEN current_date ELSE prazopagamento end into prazopagamento FROM pedcabpgto where pedcabpgto.estab=:NEW.ESTAB AND pedcabpgto.SERIE=:NEW.SERIE AND pedcabpgto.NUMERO=:NEW.NUMERO;
  ...
  BEGIN
   ...
   AS DOCPENDENTE,PERCENTUAL1,PERCENTUAL2, INSECAOTAXA,ESTABORIGEM,DTVECTOORIGEM,REGIAO,COD_PRODUTOR,NOME_PRODUTOR,CNPJF_PRODUTOR,INSCESTAD_PRODUTOR,OS_TRIANGULAR
    ...
    v_COD_PRODUTOR,v_NOME_PRODUTOR,v_CNPJF_PRODUTOR,v_INSCESTAD_PRODUTOR,v_triangular
    from pedcab_u
   ...
   contrato_edit,NRTICKET,INSECAOTAXA,ESTABORIGEM, ATIVO_OS,REGIAO,COD_PRODUTOR,NOME_PRODUTOR,CNPJF_PRODUTOR,INSCESTAD_PRODUTOR,OS_TRIANGULAR)   
