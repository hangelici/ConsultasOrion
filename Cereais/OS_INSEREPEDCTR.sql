...
v_COD_PRODUTOR NUMBER;
v_NOME_PRODUTOR VARCHAR2(100);
v_CNPJF_PRODUTOR VARCHAR2(20);
v_INSCESTAD_PRODUTOR VARCHAR2(20);
v_dtlimite DATE;
v_dtmov DATE;
...
BEGIN
 ...
 --  SELECT CASE WHEN (FORMAPGTO IN (24,22,20) and (prazopagamento <= current_date + 2)) THEN current_date ELSE prazopagamento end into prazopagamento FROM pedcabpgto where pedcabpgto.estab=:NEW.ESTAB AND pedcabpgto.SERIE=:NEW.SERIE AND pedcabpgto.NUMERO=:NEW.NUMERO;
  ...
  BEGIN
   ...
   select  TIPOFRETES,OS_TIPOENTREGA,TIPOPGTO,TIPOPESSOA,COALESCE(QTRIGO,'Sem_Padrao'),CORRETOR1,CORRETOR2,VLRCORRETOR1,VLRCORRETOR2,NRTICKET,COALESCE(DOCPENDENTE,'N')
   AS DOCPENDENTE,PERCENTUAL1,PERCENTUAL2, INSECAOTAXA,ESTABORIGEM,DTVECTOORIGEM,REGIAO,COD_PRODUTOR,NOME_PRODUTOR,CNPJF_PRODUTOR,INSCESTAD_PRODUTOR
    INTO TIPOFRETES,V_TIPOENTREGA,TIPOPGTO,TIPOPESSOA,QTRIGO,CORRETOR1,CORRETOR2,VLRCORRETOR1,VLRCORRETOR2,NRTICKET,P_ATIVO,PERCENTUAL1,PERCENTUAL2, P_INSECAOTAXA,P_ESTABORIGEM,v_prazo,v_regiao,
    v_COD_PRODUTOR,v_NOME_PRODUTOR,v_CNPJF_PRODUTOR,v_INSCESTAD_PRODUTOR
    from pedcab_u
   ...
   INSERT INTO CONTRATO_U (ESTAB,CONTRATO,STATUSASS,statusaprov,OBS,tipofretes,tipoentrega,tipopgto,tipopessoa,dtinicioent,qtrigo,statusasse,statusfat,dtemissaoori,contrato_edit,NRTICKET,INSECAOTAXA,ESTABORIGEM, ATIVO_OS,REGIAO,COD_PRODUTOR,NOME_PRODUTOR,CNPJF_PRODUTOR,INSCESTAD_PRODUTOR)   
