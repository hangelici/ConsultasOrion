WITH REMESSA_CF AS (
    ...
    (
        SELECT
        -- ESTAB E SEQNOTA SÃO DAS NOTAS DE ORIGEM DA REMESSA DE COMPRA
        ...
        WHERE
        U_TIPOOP.U_TIPOOP_ID IN (57) 
        AND NFCAB.DTEMISSAO BETWEEN (:DTINI) AND (:DTFIM)
        AND NFITEMIMPOSTO.IMPOSTO IN (4,5)
        ...
        NFCAB.SEQNOTA,
        NFCAB.NOTA,
        NFCAB.ESTAB AS ESTABORIGEM,
        NFCAB.SEQNOTA AS SEQNOTAORIGEM,
        ((CASE WHEN NFITEMIMPOSTO.IMPOSTO = 5 THEN NFITEMIMPOSTO.ALIQUOTA ELSE 0 END)) COFINS,
        ((CASE WHEN NFITEMIMPOSTO.IMPOSTO = 4 THEN NFITEMIMPOSTO.ALIQUOTA ELSE 0 END)) PIS
        FROM NFCAB
        ...
        INNER JOIN U_TIPOOP ON U_TIPOOP.U_TIPOOP_ID = NFCFG_U.U_TIPOOP_ID
        INNER JOIN NFITEMIMPOSTO ON
            NFITEMIMPOSTO.ESTAB = NFITEM.ESTAB
            AND NFITEMIMPOSTO.SEQNOTA = NFITEM.SEQNOTA
            AND NFITEMIMPOSTO.SEQNOTAITEM = NFITEM.SEQNOTAITEM
        WHERE
        NFCAB.STATUS <> 'C'
        AND NFCAB.DTEMISSAO BETWEEN (:DTINI) AND (:DTFIM)
        AND U_TIPOOP.U_TIPOOP_ID IN (63)
        AND NFITEMIMPOSTO.IMPOSTO IN (4,5)
        GROUP BY  
        NFCAB.ESTAB,
        NFCAB.SEQNOTA,
    ...
    GROUP BY 
    ESTAB,SEQNOTA,NOTA,ESTABORIGEM,SEQNOTAORIGEM
),
NOTAS AS (
    SELECT 
    REMESSA_CF.ESTAB,
