create or replace TRIGGER OS_INSEREPEDCTR_VEND
BEFORE INSERT OR UPDATE ON PEDCAB FOR EACH ROW
DECLARE
CONTRATOD NUMBER;
ESTABD number;
CONTCONFO number;
...
BEGIN
   ...
   /*CONTRATO VENDA*/
    IF :NEW.PEDIDOCONF IN (250)  AND :NEW.ETAPA > 3 AND :NEW.PODEFATURAR='S' AND :NEW.STATUS='N' AND :NEW.PROCURADOR IS NULL THEN 
