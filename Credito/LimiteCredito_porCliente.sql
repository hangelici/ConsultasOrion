...
FROM PESSOAREPRE
WHERE 
    (:REPRESENT IS NOT NULL AND PESSOAREPRE.REPRESENT = :REPRESENT)
...
FROM CONTRATO
INNER JOIN REPRESENTANTE ON REPRESENTANTE.NUMEROCM = CONTRATO.NUMEROCM 
...
INNER JOIN REPRESENTANTE ON REPRESENTANTE.NUMEROCM = CONTAMOV.NUMEROCM 
...
AND (0 IN (:PESSOA) OR CONTAMOV.NUMEROCM IN (:PESSOA))
AND ((:REPRESENT) IS NULL OR PESSOAREPRE.REPRESENT = :REPRESENT)
and
exists 
(
select
