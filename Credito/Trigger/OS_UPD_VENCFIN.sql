...
FOR VALORES IN (
    ...
    INNER JOIN U_ALTCONLIMITE U
       ...
       AND U.ESTAB = 1
    WHERE
        CONTAMOV.ESTABFINRESPONSAVEL NOT IN (
            SELECT ESTAB
            FROM U_ALTCONLIMITE
            WHERE ESTAB <> 1
        )
...
LOOP
   ...
   OR VALORES.ENTRFIM IS NOT NULL THEN
